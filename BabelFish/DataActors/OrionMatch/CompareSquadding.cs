...
namespace Scopos.BabelFish.DataActors.OrionMatch {
    ...
    public class CompareSquadding : IComparer<Squadding> {
        ...
        public int Compare( Squadding x, Squadding y ) {
            ...
            int compare = 0;
            Individual X, Y;
            SquaddingAssignmentFiringPoint saX, saY;
            ...
            switch (Method) {
                ...
                case CompareMethod.RELAY_FIRINGPOINT_DISPLAYNAME:
					if (x.SquaddingAssignment is not SquaddingAssignmentFiringPoint
						|| y.SquaddingAssignment is not SquaddingAssignmentFiringPoint)
						throw new ArgumentException( "This ain't no disco! Firing Points only!" );

					saX = (SquaddingAssignmentFiringPoint)x.SquaddingAssignment;
                    saY = (SquaddingAssignmentFiringPoint)y.SquaddingAssignment;

                    compare = saX.Relay.CompareToAsIntegers( saY.Relay );
                    if (compare == 0) {
                        compare = saX.FiringPoint.CompareToAsIntegers( saY.FiringPoint );
                        if (compare == 0) {
                            compare = x.Participant.DisplayName.CompareTo( y.Participant.DisplayName );
                        }
                    }
