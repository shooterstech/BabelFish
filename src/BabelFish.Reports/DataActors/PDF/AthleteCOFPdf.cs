...
namespace Scopos.BabelFish.DataActors.PDF {
    public class AthleteCOFPdf : PdfGenerator {
        ...
        /// <summary>
        /// Optional text to include in the header with demographic information about the athlete. Uses string interpolation with feidls
        /// such  as {CompetitorNumber}, {Club}, and {Country} as variables to be replaced with the athlete's information.
        /// </summary>
        public string DemographicText { get; set; } = string.Empty;
            ...
            .Row( row => {
                row.RelativeItem().Column( column => {
                    column.Item().ShrinkVertical().Row( row => {
                        row.RelativeItem( 2 ).AlignLeft().Text( ResultCOF.Participant.DisplayName ).SemiBold().FontSize( 14 ).FontColor( this.DefaultHeaderTextColor );
                    ...
                    column.Item().Text( $"{ResultCOF.MatchName} | {StringFormatting.SingleDate( ResultCOF.LocalDate )}" ).SemiBold().FontSize( 11 ).FontColor( this.DefaultHeaderTextColor );
                    column.Item().Text( $"Course of Fire: {CourseOfFire.CommonName}" ).SemiBold().FontSize( 11 ).FontColor( this.DefaultHeaderTextColor );
                    column.Item().Text( $"Status: {ResultCOF.Status} | Printed at {StringFormatting.SingleDateTime( DateTime.Now )}" ).FontSize( 11 ).FontColor( this.DefaultHeaderTextColor );
                    if (!string.IsNullOrEmpty( DemographicText )) {
                        column.Item().Text( DemographicText.Replace( this.ResultCOF ) ).FontSize( 11 ).FontColor( this.DefaultHeaderTextColor );
                    }
                } );
            } );
        }
