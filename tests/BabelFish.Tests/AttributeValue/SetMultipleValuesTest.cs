...
namespace Scopos.BabelFish.Tests.AttributeValue {
    [TestClass]
    public class SetMultipleValuesTest : BaseTestClass {


        [TestMethod]
        public async Task SetMultiplePhoneNumbers() {

            var setNamePhoneNumber = SetName.Parse( "v2.0:orion:Phone Number" );
            ...
            List<SetName> myAttributes = new List<SetName>() { setNamePhoneNumber };
            ...
            var phoneNumberAttrValue = await Scopos.BabelFish.DataModel.AttributeValue.AttributeValue.CreateAsync( setNamePhoneNumber );
        ...
        public async Task SetMultipleSocialMediaAccounts() {
            ...
            var setNameSocialMedia = SetName.Parse( "v1.0:orion:Social Media Accounts" );
            ...
            //Retreive the user's current values.            
            //Will use a GetAttributeValueAuthenticatedRequest object in this unit test, so I can set ReturnDefaultvalues to true (it is by default false).
            List<SetName> myAttributes = new List<SetName>() { setNameSocialMedia };

            //For the purpose of this unit test, I am starting with a clean / brand new Attribute Value. Normally, in real life
            //The user's attribute value should be read first
            var socialMediaAttrValue = await Scopos.BabelFish.DataModel.AttributeValue.AttributeValue.CreateAsync( setNameSocialMedia );
