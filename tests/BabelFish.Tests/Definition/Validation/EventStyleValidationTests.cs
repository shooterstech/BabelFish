...
namespace Scopos.BabelFish.Tests.Definition.Validation {
    ...
    public class EventStyleValidationTests : BaseTestClass {
        ...
        public async Task IsEventStylesAndStageStylesValid() {
            ...
            //Populate both
            eventStyle.StageStyles.Add( SetName.Parse( "v1.0:ntparc:Sporter Air Rifle Prone" ) );
            eventStyle.EventStyles.Add( SetName.Parse( "v1.0:nra:BB Gun" ) );
            valid = await validation.IsSatisfiedByAsync( eventStyle );
            Assert.IsFalse( valid );
            Assert.IsTrue( validation.Messages.Count > 0 );
        }
        ...
        public async Task IsEventStylesValid() {
            ...
            eventStyle.EventStyles.Clear();
            eventStyle.EventStyles.Add( SetName.Parse( "v1.0:orion:not a real set name" ) );
            valid = await validation.IsSatisfiedByAsync( eventStyle );
            Assert.IsFalse( valid );
            Assert.IsTrue( validation.Messages.Count > 0 );
        ...
        public async Task IsRelatedEventStylesValid() {
            ...
            //A fake set name should fail
            eventStyle.RelatedEventStyles.Clear();
            eventStyle.RelatedEventStyles.Add( SetName.Parse( "v1.0:orion:not a real set name" ) );
            valid = await validation.IsSatisfiedByAsync( eventStyle );
            Assert.IsFalse( valid );
            Assert.IsTrue( validation.Messages.Count > 0 );
            ...
            //A real set name should pass
            eventStyle.RelatedEventStyles.Clear();
            eventStyle.RelatedEventStyles.Add( SetName.Parse( "v1.0:nra:BB Gun" ) );
            valid = await validation.IsSatisfiedByAsync( eventStyle );
            Assert.IsTrue( valid );
            Assert.IsTrue( validation.Messages.Count == 0 );
        }
        ...
        public async Task IsStageStylesValid() {
            ...
            //A fake set name should fail
            eventStyle.StageStyles.Clear();
            eventStyle.StageStyles.Add( SetName.Parse( "v1.0:orion:not a real set name" ) );
            valid = await validation.IsSatisfiedByAsync( eventStyle );
            Assert.IsFalse( valid );
            Assert.IsTrue( validation.Messages.Count > 0 );
            ...
            //A real set name should pass
            eventStyle.StageStyles.Clear();
            eventStyle.StageStyles.Add( SetName.Parse( "v1.0:ntparc:Sporter Air Rifle Standing" ) );
            valid = await validation.IsSatisfiedByAsync( eventStyle );
            Assert.IsTrue( valid );
            Assert.IsTrue( validation.Messages.Count == 0 );
        }
        ...
        public async Task IsSimpleCOFValid() {
            ...
            eventStyle.SimpleCOFs[0].CourseOfFireDef = SetName.Parse( "v1.0:orion:not a real set name" );
            valid = await validation.IsSatisfiedByAsync( eventStyle );
            Assert.IsFalse( valid );
            Assert.IsTrue( validation.Messages.Count > 0 );
            ...
            eventStyle = eventStyleOrig.Clone();
            eventStyle.SimpleCOFs[0].Components[0].StageStyleDef = SetName.Parse( "v1.0:nra:BB Gun Kneeling" );
            valid = await validation.IsSatisfiedByAsync( eventStyle );
            Assert.IsFalse( valid );
            Assert.IsTrue( validation.Messages.Count > 0 );
        }
